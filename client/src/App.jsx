...
// --- CONTRACT DETAILS ---
const P2P_STAKING_ADDRESS = '0xDd58b36d71f23a1824bc28288d25a2A2f87eD3f3';
const MTUSDT_ADDRESS = '0x2AEF42da66D2ef5C88C45953d6FDDc25DE23dd23';

// --- YOUR UPGRADED & VERIFIED ABI ---
...
const BSC_SCAN_URL = 'https://testnet.bscscan.com';
...
function App() {
    ...
    try {
      const [info, stats, history, ...teamLevels] = await Promise.all([
        ...
        ...Array.from({ length: 10 }, (_, i) =>
          contract.getTeamCount(account, i + 1)
        ),
      ]);
  ...
  const handleRegister = async () => {
    try {
      ...
      alert('Step 1/2: Approving USDT spending...');
      const approveTx = await usdtContract.approve(
        P2P_STAKING_ADDRESS,
        ethers.parseUnits('125', 18)
      );
      await approveTx.wait();
      alert('Step 2/2: Registering your account...');
      const registerTx = await contract.register(referrer);
  ...
  const renderContent = () => {
    ...
    if (!isRegistered) {
      return (
        <div className="action-card">
          <h3>Join Now</h3>
          <p>Purchase the 125 USDT package to start earning.</p>
    ...
    switch (activeTab) {
      ...
      case 'history':
        return (
          <IncomeHistoryView history={incomeHistory} currentUser={account} />
        );
      case 'awards':
        return (
          <AwardsView
            directs={userStats.directs}
      ...
      default:
        return (
          <DashboardView userInfo={userInfo} onClaim={handleClaimTokens} />
        );
    }
  };
    ...
    return (
      <div className="main-container">
        <div className="connect-wallet-container">
          <img
            src="https://i.ibb.co/0VYCDPfP/logo.png "
            alt="Company Logo"
            className="logo"
          />
    ...
    <div className="referral-container">
        ...
        <div className="dashboard-grid">
          <div className="stat-card">
            ...
            <p>{parseFloat(userStats.totalEarnings).toFixed(4)} USDT</p>
          </div>
          <div className="stat-card">
            <h3>Total Team</h3>
            <p>{userStats.totalTeam}</p>
          </div>
          <div className="stat-card">
            <h3>Total Directs</h3>
            <p>{userStats.directs}</p>
          </div>
        </div>
...
const DashboardView = ({ userInfo, onClaim }) => (
  <>
    <div className="dashboard-grid">
      <div className="stat-card">
        <h3>Total Stake</h3>
        <p>{userInfo?.totalReward} P2P</p>
      </div>
      <div className="stat-card">
        <h3>Claimed Tokens</h3>
        <p>{parseFloat(userInfo?.totalClaimed).toFixed(4)} P2P</p>
      </div>
      <div className="stat-card">
        <h3>Remaining Tokens</h3>
        <p>
          {(
            (userInfo?.totalReward || 0) - (userInfo?.totalClaimed || 0)
          ).toFixed(4)}{' '}
          P2P
        </p>
      </div>
      <div className="stat-card">
        <h3>Lock Period Ends</h3>
        <p style={{ fontSize: '16px' }}>{userInfo?.lockEndDate}</p>
      </div>
    </div>
    <div className="action-card">
      <h3>Claim Your Monthly Rewards</h3>
      <p>
        You can claim 5% of your staked tokens every 30 days after the lock
        period ends.
      </p>
          ...
          [...history].reverse().map((item, index) => (
            <tr key={index}>
              ...
              <td>
                ...
                {Number(item.incomeType) === 0 ? 'USDT' : 'P2P'}
              </td>
              <td>
                {Number(item.incomeType) === 0
                  ? `${item.fromUser.substring(0, 6)}...`
                  : 'Self'}
              </td>
              <td>
              ...
              <td>
                <a
                  href={`${BSC_SCAN_URL}/address/${P2P_STAKING_ADDRESS}`}
                  target="_blank"
...
const AwardsView = ({ directs, teamByLevel, onClaim }) => {
  const awards = [
    ...
    {
      id: 2,
      name: '50 Team in Level 2',
      required: 50,
      current: teamByLevel[1] || 0,
      reward: '5 P2P',
    },
    {
      id: 3,
      name: '150 Team in Level 3',
      required: 150,
      current: teamByLevel[2] || 0,
      reward: '15 P2P',
    },
  ];
